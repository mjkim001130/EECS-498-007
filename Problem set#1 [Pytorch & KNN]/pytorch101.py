...
def mutate_tensor(
    ...
    # zip은 2개 이상의 iterable한 객체를 병렬로 묶어주는 함수이다.
    # ex) list1 = [1, 2, 3], list2 = [a, b, c]
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    # torch.arange(start, stop, step) : start부터 stop까지 step만큼 증가
    # Find the first multiple of 10
    first_multiple_of_ten = (start + 9) // 10 * 10

    if first_multiple_of_ten > stop:
        x = torch.empty(0, dtype=torch.float64)
    else:
        x = torch.arange(first_multiple_of_ten, stop + 1, 10, dtype=torch.float64)
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    y = torch.cat([x_reshaped[0], x_reshaped[1]], dim=1)    
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    y = x.clone()
    min_index = torch.argmin(y, dim=1)
    y[torch.arange(y.shape[0]), min_index] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    # 1. 각 열의 평균 계산
    column_means = x.sum(dim=0) / M  # (N,)
    
    # 2. 표준 편차 계산 (Bessel's correction 적용)
    deviations = x - column_means  # (M, N)
    squared_deviations = deviations ** 2  # (M, N)
    variance = squared_deviations.sum(dim=0) / (M - 1)  # (N,)
    std_devs = variance.sqrt()  # (N,)
    
    # 3. 표준 편차가 0인 경우 1로 설정하여 나누기 방지
    std_devs[std_devs == 0] = 1.0
    
    # 4. 정규화: 각 열에서 평균을 빼고 표준 편차로 나누기
    y = (x - column_means) / std_devs
