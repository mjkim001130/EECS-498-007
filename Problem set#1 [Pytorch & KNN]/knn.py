...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        diff = x_train_flatten[i] - x_test_flatten
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    train_square = torch.sum(x_train_flatten ** 2, dim=1, keepdim=True)
    test_square = torch.sum(x_test_flatten ** 2, dim=1)

    dot_product = x_train_flatten @ x_test_flatten.t()

    dists = train_square - 2 * dot_product + test_square
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        ...
        _, idx = torch.topk(dists[:, i], k=k, largest=False)

        # Get labels of the k nearest neighbors
        nearst_labels =  y_train[idx]

        # Count
        cnt = torch.bincount(nearst_labels)

        y_pred[i] = torch.argmax(cnt)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    # 하나의 fold를 validation set으로 사용하고, 나머지를 training set
    # 나머지 fold를 합쳐야함 -> torch.cat
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    ...
    # k_choices 리스트에 있는 k값들을 반복해서 corss validation을 수행
    for k in k_choices:
        # 각 k에 대해 accuracy를 저장할 리스트 
        k_to_accuracies[k] = []

        # num_folds만큼 반복
        for i in range(num_folds):
            # i번째 fold를 validation set으로 사용
            x_val = x_train_folds[i]
            y_val = y_train_folds[i]

            # 나머지는 torch.cat을 사용하여 합침
            x_train_fold = []
            y_train_fold = []

            for j in range(num_folds):
                if j != i: # i번째 fold를 제외한 나머지 fold를 합침
                    x_train_fold.append(x_train_folds[j])
                    y_train_fold.append(y_train_folds[j])

            x_train_fold = torch.cat(x_train_fold, dim=0)
            y_train_fold = torch.cat(y_train_fold, dim=0)

            # knn classifier
            knn  = KnnClassifier(x_train_fold, y_train_fold)

            # accuracy 계산
            y_val_pred = knn.predict(x_val, k)

            accuracy = (y_val == y_val_pred).float().mean().item() * 100
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for i in k_to_accuracies:
        acc = k_to_accuracies[i]
        avg_acc = sum(acc) / len(acc)

        # k에 대해 avg_acc가 best_acc보다 크면 update
        if avg_acc > best_acc or (avg_acc == best_acc and i < best_k):
            best_k = i
            best_acc = avg_acc
